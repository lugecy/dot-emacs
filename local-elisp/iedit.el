...
;;;###autoload
(defun iedit-mode (&optional arg func)
  "Toggle iedit mode.
...
\\{iedit-mode-map}"
  ...
  (if iedit-mode
    ...
    (let ((occurrence nil))
      ...
      (deactivate-mark)
      (iedit-start occurrence func))))

(defun iedit-start (occurrence-exp &optional collect-func)
  ...
  (unless collect-func
    (setq collect-func (lambda (exp)
                              (goto-char (point-min))
                              (let (overlays)
                                (while (search-forward exp nil t)
                                  (push (iedit-make-occurrence-overlay (match-beginning 0) (match-end 0))
                                        overlays))
                                (nreverse overlays)))))
  (save-excursion
    (setq iedit-occurrences-overlays (funcall collect-func occurrence-exp))
