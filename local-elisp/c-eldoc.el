...
;; run the first time for macros
  (let ((output-buffer (cache-gethash (or buffer-file-name (current-buffer)) c-eldoc-buffers)))
    (if output-buffer output-buffer
      (let* ((this-name (concat "*" buffer-file-name "-preprocessed*"))
             (preprocessor-command (concat c-eldoc-cpp-command " "
                                           ...
                                           c-eldoc-includes " "
                                           buffer-file-name))
             (cache-key (or buffer-file-name (current-buffer)))
        ...
        (setq preprocessor-command (concat c-eldoc-cpp-command " "
                                           ...
                                           buffer-file-name))
        (call-process-shell-command preprocessor-command nil output-buffer nil)
        (cache-puthash cache-key output-buffer c-eldoc-buffers)
        output-buffer))))
...
(defvar c-eldoc-marker-cache nil)
...
(defun c-eldoc-print-current-symbol-info ()
  ...
  (let* ((current-function-cons (c-eldoc-function-and-argument (- (point) 1000)))
         ...
         (current-function-regexp (concat "\\<" current-function "[ )\t\n]*("))
    ...
    (when (and current-function
      ...
      (when (setq tag-buffer (c-eldoc-get-buffer current-function))
        ...
        (prog1
            ...
            (when (or (and (markerp (cdr c-eldoc-marker-cache))
                           (eq tag-buffer (marker-buffer (cdr c-eldoc-marker-cache)))
                           (equal current-function (car c-eldoc-marker-cache))
                           (goto-char (cdr c-eldoc-marker-cache)))
                      (condition-case nil
                          (progn
              ...
              (setq c-eldoc-marker-cache (cons current-function (point-marker)))
