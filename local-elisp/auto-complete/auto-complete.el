...
(defcustom ac-enable-prefix-visual nil
  "Non-nil means current prefix visualize."
  :type 'boolean
  :group 'auto-complete)
...
(defface ac-prefix-overlay-face
  '((t (:background "DarkSlateGrey")))
  "Face for ac-prefix-overlay"
  :group 'auto-complete)
...
(defvar ac-last-expand-point nil
  "Last point of execute complete or expand command.")
...
(defun ac-compile-sources (sources)
  ...
  (loop for source in sources
        for src-name = (and (symbolp source) (intern-soft source))
        ...
        (flet ((add-attribute (name value &optional append) (add-to-list 'source (cons name value) append)))
          (add-attribute 'src-name src-name)
          ;; prefix
...
(defun ac-put-prefix-overlay ()
  (unless ac-prefix-overlay
    (let (newline)
      ...
      (when ac-enable-prefix-visual
        (overlay-put ac-prefix-overlay 'face 'ac-prefix-overlay-face))
...
(defun ac-prefix (requires ignore-list)
  (loop with current = (point)
        ...
        if (and (null prefix-def)
                (not (memq prefix miss-prefix-func)))
        do
...
(defun ac-candidates-1 (source)
  (let* ((do-cache (assq 'cache source))
         ...
         (src-name (assoc-default 'src-name source))
...
(defun ac-candidates ()
  ...
  (loop with completion-ignore-case = (or (eq ac-ignore-case t)
        ...
        if (>= prefix-len (or (assoc-default 'requires source) 1))
...
(defun ac-cleanup ()
  ...
  (when (and ac-use-comphist ac-comphist
             ac-selected-candidate ac-last-expand-point)
    (ac-comphist-add ac-comphist
                     ac-selected-candidate
                     (- ac-last-expand-point ac-point)))
  (ac-deactivate-completing-map)
...
(defun ac-update (&optional force no-gen-cand)
...
(defun ac-expand ()
  ...
  (unless (ac-expand-common)
    (let ((string (ac-selected-candidate)))
      (when string
        (setq ac-last-expand-point ac-last-point)
...
(defun ac-expand-common ()
  ...
  (if (and ac-dwim ac-dwim-enable)
    ...
    (when (and (ac-inline-live-p)
      ...
      (when (equal ac-common-part (ac-selected-candidate))
        (setq ac-last-expand-point ac-last-point))
