...
(defun jaunte-with-source (source)
  (let ((hint-list (funcall (assoc-default 'hints source))))
    (jaunte-cycle-reset 'jaunte-hint-faces)
    (jaunte-make-hints hint-list)
    (unwind-protect
        (let (k key)
          (while (not (null jaunte--hints))
            (setq k (read-event (concat "Jaunte to " key)))
            (if (and (not (null key))
              ...
              (setq key (concat key (char-to-string k)))
              (jaunte-search key))
            (when (= 1 (length jaunte--hints))
              (anything-aif (assoc-default 'action source)
                  (funcall it (car jaunte--hints))
                (jaunte-to (car jaunte--hints)))
              (jaunte-remove-hints))))
      (jaunte-remove-hints))))

(defun jaunte-make-hints (hint-list)
  (setq jaunte--hints
        (loop for i from 0
              for cell in hint-list
              for key = (jaunte-make-key i)
              for pos = (if (consp cell) (car cell) cell)
              for win = (if (consp cell) (cdr cell) (selected-window))
              collect (jaunte-make-hint key
                                        (with-selected-window win
                                          (jaunte-make-overlay pos key))
                                        win pos))))

(defun jaunte-all-window ()
  (interactive)
  (jaunte-with-source '((hints . jaunte-all-window-hints))))

(defun jaunte-all-window-hints ()
  (let (lst)
    (mapcar
     (lambda (window)
       (save-excursion
         (save-window-excursion
           ...
           (let ((point (if (looking-at "\\w")
                 ...
                 (window-end (window-end window)))
             (while (< point window-end)
               (push (cons point window) lst)
               (jaunte-forward-word)
               (setq point (point)))))))
     (window-list))
    (nreverse lst)))

(defun jaunte-twittering ()
  (interactive)
  (jaunte-with-source
   '((hints . (lambda () (twit-hah-uri-list (selected-window))))
     (action . (lambda (pos)
                 (save-excursion
                   (jaunte-to pos)
                   (twittering-enter)))))))
