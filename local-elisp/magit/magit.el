...
(defun magit-cmd-output (cmd args)
  (let* ((args (if magit-use-cygwin-git-flag
                   (mapcar #'magit-escape-for-w32 args)
                 args))
...
(defun magit-run* (cmd-and-args
  ...
  (let ((cmd (car cmd-and-args))
	(args (if magit-use-cygwin-git-flag
                  (mapcar #'magit-escape-for-w32 (cdr cmd-and-args))
                (cdr cmd-and-args)))
...
(defun magit-interactive-rebase ()
  ...
  (let* ((section (get-text-property (point) 'magit-section))
    ...
    (setenv "GIT_EDITOR" (locate-file "emacsclient" exec-path (when (eq system-type 'windows-nt) '(".exe"))))
    (unwind-protect
