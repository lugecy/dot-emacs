...
(defvar thing-opt-highlight-overlay nil)
(defface thing-opt-highlight-face
  '((t (:background "magenta")))
  "Face for thing-opt highlight")
...
(defun kill-region-dwim-1 (function)
  (if (and transient-mark-mode mark-active)
    ...
    (let* ((c (if (fboundp 'kill-region-dwim-popup)
                  (kill-region-dwim-popup)
                (read-char)))
      ...
      (cond
       (bounds
        (let ((beg (car bounds))
          ...
          (overlay-put (setq thing-opt-highlight-overlay (make-overlay beg end))
                       'face 'thing-opt-highlight-face)
          (add-hook 'pre-command-hook 'thing-opt-unhighlight)
...
(defun thing-opt-unhighlight ()
  "unhighlight thing-opt overlay"
  (when thing-opt-highlight-overlay
    (delete-overlay thing-opt-highlight-overlay)
    (setq thing-opt-highlight-overlay nil)
    (remove-hook 'pre-command-hook 'thing-opt-unhighlight)))
...
(eval-after-load "popup"
  '(defun kill-region-dwim-popup ()
     (let* ((mesg (loop for (c . target) in kill-thing-map
                        concat (format "(%c):%s\n" c target)))
            (tip (popup-tip mesg :nowait t :height (length kill-thing-map)))
            char)
       (unwind-protect
           (setq char (read-char))
         (popup-delete tip))
