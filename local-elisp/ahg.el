...
;;-----------------------------------------------------------------------------
;; For windows by lugecy
;;-----------------------------------------------------------------------------
(defcustom ahg-commit-use-file nil
  "Use temporary log file when commit.")

(defcustom ahg-hg-command-generic-arguments nil
  "ahg-generic-command arguments for hg global.")
...
(defun ahg-commit-callback ()
  ...
  (let* ((msg (ahg-parse-commit-message))
    ...
    (let ((args (append (if log-file (list "-l" log-file) (list "-m" msg))
                        (log-edit-files))))
      (ahg-generic-command
       "commit" args
       (lexical-let ((aroot (ahg-root))
         ...
         (lambda (process status)
           (if (string= status "finished\n")
               (progn
                 ...
                 (when log-file
                   (delete-file log-file))           ;; add by lugecy
  ...
  (let ((buffer (get-buffer-create
    ...
    (ahg-generic-command
     ...
     (lexical-let ((dn (or ahg-dir-name-for-log-command default-directory))
                   (ahg-log-with-patch-flg ahg-log-with-patch-flg))
       (lambda (process status)
         (if (string= status "finished\n")
             (progn
               (pop-to-buffer (process-buffer process))
               (ahg-format-log-buffer)
               (if ahg-log-with-patch-flg (ahg-diff-mode) (ahg-log-mode))
               (goto-char (point-min))
               (let ((inhibit-read-only t))
                 (insert
                  (propertize "hg log for " 'face ahg-header-line-face)
    ...
    (labels ((next () (beginning-of-line) (forward-line 1)))
      (while (not (eobp))
        ...
        (while (and (looking-at "^\\(\t\\|$\\)") (not (eobp)))
          ...
          (next))
        ;; skip patch part
        (unless (or (looking-at "^[0-9]+:[0-9a-zA-Z]+$") (eobp))
          (insert "\n")
          (while (not (or (looking-at "^[0-9]+:[0-9a-zA-Z]+$") (eobp)))
            (next)))
        (insert "\n\n")
        ))))
...
(defun ahg-mq-edit-series ()
  ...
  (let* ((some-patches-applied
         (with-temp-buffer
           (when (= (ahg-call-process "tip" (list "--template" (if (featurep 'dos-w32)
                                                                   "\\{tags\\}"
                                                                 "{tags}")))
                    0)
  ...
  (with-temp-buffer
    (let ((process-environment (cons "LANG=" process-environment)))
      (if (= (ahg-call-process "parents"
                               (list "-r" rev "--template" (if (featurep 'dos-w32)
                                                               "\\{rev\\}"
                                                             "{rev}")))
             0)
          (buffer-string)
  ...
  (let ((lang (getenv "LANG")))
    ...
    (let ((process
           (apply (if use-shell 'start-process-shell-command 'start-process)
                  ...
                  ahg-hg-command
                  (append
                   ahg-hg-command-generic-arguments
...
(defun ahg-uncommitted-changes-p (&optional root)
  (with-temp-buffer
    (let ((global-opts (when root (list "-R" root))))
      (when (= (ahg-call-process "id" (list "--config" "defaults.identify=" "-n") global-opts) 0)
    ...
    (log-edit
     ...
     (if (version< emacs-version "23.0") file-list-function
       (list (cons 'log-edit-listfun file-list-function)
             (cons 'log-edit-diff-function #'ahg-diff)))
     buffer)))
...
;; For windows by lugecy
;;-----------------------------------------------------------------------------
(defun ahg-make-commit-logfile (msg)
  "make commit logfile from message.
   return tmp file name"
  (let ((tmp-file (make-temp-name (expand-file-name "ahg"
                                                    temporary-file-directory))))
    (with-temp-file tmp-file
      (let ((buffer-file-coding-system (if ahg-subprocess-coding-system
